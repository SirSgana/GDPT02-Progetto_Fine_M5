...
////LookAroundOriginale dove l'Enemy si ferma solo per 3 secondi
//public class LookAroundState : BaseState
//{
//    //Variabile necessaria per riuscire a farsì che il lookaround funzioni 
//    private float timer; //Se usassi lo stats il timer sarebbe già a 0 ed il lookaround non funzionerebbe 
...
//        timer = enemy.stats.rotationInterval;
//    }
//    public override void StateUpdate() => timer -= Time.deltaTime;
//    public override void StateExit() => enemy.agent.isStopped = false;
//    public override void CheckTransition()
//    {
//        if (enemy.CanSeePlayer())
...
//LookAround Update state con rotazione ma non mi piaceva 

////Creo una rotazione graduale
//enemy.transform.rotation = Quaternion.Slerp(enemy.transform.rotation, targetRotation, rotationSpeed * Time.deltaTime);

////Se ha raggiunto il targetRotationSX
//if (Quaternion.Angle(enemy.transform.rotation, targetRotation) < 1f)
//{
//    if (lookStep == 0) //Una volta finito di guardare a sinistra
//    {
//        lookStep = 1; //Inizia la rotazione a destra
//        //Modifico la rotazione di target ora è +90 a destra
//        targetRotation = originalRotation * Quaternion.Euler(0, 90, 0); 
//    }
//    else if (lookStep == 1) //Ha finito di guardare a destra
//    {
//        lookStep = 2; //Fine LookAround
//    }
//}